# Progressive Dragonfly routing for PR 5

## Scope

PR 5 introduces a tested Dragonfly routing core for the `rnic-ss` study. It
does not change the existing Clos experiment and it does not reuse the legacy
`UGAL_L` implementation.
...
- the complete nine-placement 64-of-72 endpoint and logical-role rotation; and
- the RNIC-SS load-observation timestamp correction.

Not yet enabled in a production simulator executable:

- a Dragonfly Rosetta switch adapter that performs the route lookup after the
  modeled switch-pipeline delay;
- packet-owned route-state plumbing for `rnic-ss` and the ordered DCQCN
  control;
- physical credit and downstream-advertisement packets;
- phase-separated buffering/PFC treatment for a lossless DCQCN baseline; and
- wiring the balanced 64-active-rank placement into GOAL/runtime endpoint
  binding.

Until those gates are complete, the new core is not used to generate paper
performance results. The controlled Clos slide remains the current evidence.

## Suggested review order

1. `docs/rnic-dragonfly-pr5-plan.md`
...
10. the RNIC-SS timestamp change and its focused tests
