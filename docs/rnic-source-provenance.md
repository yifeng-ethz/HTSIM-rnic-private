...
| Every Clos tier uses the `ns-tm3` behavioral model with true ingress/egress VoQs. | Current discussion; NERSC slide 3 only supports TM3+ as a spraying-capable example | Exact switch scheduling is a declared simulator contract, not a reverse-engineered ASIC claim. `ns-tm3` names the model; Tomahawk 3 names the hardware family used as provenance. |
