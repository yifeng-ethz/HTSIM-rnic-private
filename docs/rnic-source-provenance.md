...
| --- | --- | --- |
| CN means **collective network**, never congestion control. Public profiles are `rnic-cn`, `rnic-ss`, `rnic-nn`, and `rnic-nn-fluid`. | Current discussion and simulator contract | Do not expose `rnic-cc` or retain legacy `rnic-null` naming. `rnic-ss` is explicitly Slingshot-like, not a proprietary product reproduction. |
| Every CN flow declares `nflow=1` before DATA; DATA is ineligible until an in-band ACCEPT; the sender then steps directly from zero to the returned grant. | Current discussion; paper pp. 4--6, Figure 3 and Sections 3.2, 4.1--4.2; formal appendix lines 193--211 | `nflow` is the sole DECLARE input to RX CCA. Optional `collective_id` and `expected_fan_in` are trace-only and are discarded before the membership batch; they never pre-count a collective or alter `N_hat`. No slow start, rate probing, additive ramp, lookback estimate, or out-of-band oracle in `rnic-cn`. |
...
| CN's PRBS pacer is one shared sender-side scheduler over per-flow heads; ETA is stamped at physical route injection after source serialization. | Patent pp. 9 and 14, Appendices B and F; paper p. 4, Figure 1 | Node-distinct reproducible LFSR phases approximate independent renewal streams without synchronizing sender sequences. Packet-specific no-load transit is added to the source-serializer completion boundary because the physical HTSIM route begins there. It includes pipe/switch latency and every remaining `ns-tm3` egress serialization at the exact wire extent: one leaf-to-RNIC downlink for same-leaf traffic, or two equal-rate leaf-spine links plus one leaf-to-RNIC downlink for cross-leaf traffic. The paper-scoped profile rejects failed-link heterogeneity and snapshots calibration inputs when it constructs the physical Clos. Mixed extents use size-normalized fixed-point event hazards so grants remain wire-byte rates; equal extents preserve the selected generator version's one-word-per-opportunity replay path exactly. PRBS v2 consumes disjoint 64-bit output blocks; v1's one-step/full-register exposure is retired because adjacent register-state draws were strongly linearly related. PRBS supplies an intended renewal-like approximation, not a configured deterministic `(sigma,rho)` arrival curve. |
| CN join waves use one common effective time under a configured homogeneous control deadline. | Current discussion's homogeneous two-tier-Clos scope; paper pp. 4--6 supplies declaration/ACCEPT/direct-grant invariants but does not specify concurrent fanout ordering. | ACCEPT/UPDATE packets remain physical and in band. Incumbents retain old grants and joiners remain gated until one non-copyable receiver controller authenticates and atomically commits the complete wave. That controller is the only membership mutation owner. Same-time feedback arrival precedes barrier activation, which precedes DATA. All packets must arrive by the declared worst-case deadline or the run is invalid. Waves are serialized. This is an explicit simulator idealization, not a heterogeneous-delay guarantee. |
| RB admission and release are driven by packet eligibility time, not FIFO arrival or PSN. Missing/late packets cannot head-of-line block unrelated eligible packets. | Patent pp. 3--5, Sections 3.3--3.5 and Figure 2; paper p. 4, Figure 2 | Admit age `0 <= q <= Delta`; equality is in-window and only `q > Delta` is late. Release at quantized `ETA + Delta`; use modular timestamp comparisons. |
...
| `rnic-cn` loss recovery is exact-gap deterministic retransmission. | Current discussion; patent Appendix C; bounded-window and post-resequencing contracts | A successor released after resequencing exposes a middle hole; RETIRE publishes the maximum original release deadline over the complete PSN sequence and exposes a tail hole without a second `Delta` wait. All same-timestamp admissions/releases precede the decision (`epsilon=0`). GAP_NACK carries the exact logical range and attempt, not simulator lifecycle or rejected ETA. Retransmission uses the same grant/PRBS service with a fresh ETA; duplicates are idempotent. A routed GAP_RESOLVED—not direct RX access—closes sender state. The bounded positive RTO begins after physical retry serialization and covers dropped DATA/absent closure only; any dropped DECLARE, ACCEPT/UPDATE, GAP_NACK, GAP_RESOLVED, or RETIRE is fatal rather than recovered by that watchdog. |
...
| Every `rnic-cn` Clos tier uses the `ns-tm3` behavioral model with true ingress/egress VoQs; `rnic-ss` uses the distinct `ns-rosetta` model. | Current discussion; NERSC slide 3 only supports TM3+ as a spraying-capable example; public Slingshot sources support mechanism classes but not proprietary control arithmetic | Exact switch scheduling is a declared simulator contract, not a reverse-engineered ASIC claim. `ns-tm3` and `ns-rosetta` name open models; Tomahawk 3 and Rosetta name hardware families used only as provenance. |
...
- Node-distinct sender PRBS phases suppress the periodic impulse peak and
  ...
  the observation window grows. PRBS alone neither undoes burstiness introduced
  by switches nor provides a hard all-time `Delta` delay bound; such a bound
  needs a separately specified deterministic arrival regulator.
...
| Legacy `htsim_clos_voq` “TM-3/VoQ” | Its README admits it is HTsim lossless-input plus lossless-output behavior, not a true VoQ traffic manager or proprietary TM3 model. No primary source specifies exact TM3 arbitration. | Do not preserve its queue behavior or numbers as golden. Record the new `ns-tm3` behavioral scheduler explicitly in every manifest. |
...
- [ ] full `ns-tm3` behavioral contract: VoQ key, arbitration policy,
