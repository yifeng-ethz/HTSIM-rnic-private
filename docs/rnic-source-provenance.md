...
| CN's PRBS pacer is one shared sender-side scheduler over per-flow heads; ETA is stamped at physical route injection after source serialization. | Patent pp. 9 and 14, Appendices B and F; paper p. 4, Figure 1 | Node-distinct reproducible LFSR phases approximate independent renewal streams without synchronizing sender sequences. Packet-specific no-load transit is added to the source-serializer completion boundary because the physical HTSIM route begins there. It includes pipe/switch latency and every remaining `ns-tm3` egress serialization at the exact wire extent: one ToR downlink for same-ToR traffic, or two aggregation-rate inter-switch links plus one ToR downlink for cross-ToR traffic. The paper-scoped profile rejects failed-link heterogeneity and snapshots calibration inputs when it constructs the physical Clos. Mixed extents use size-normalized fixed-point event hazards so grants remain wire-byte rates; equal extents preserve the selected generator version's one-word-per-opportunity replay path exactly. PRBS v2 consumes disjoint 64-bit output blocks; v1's one-step/full-register exposure is retired because adjacent register-state draws were strongly linearly related. PRBS supplies an intended renewal-like approximation, not a configured deterministic `(sigma,rho)` arrival curve. |
