...
| CN's PRBS pacer is one shared sender-side scheduler over per-flow heads; ETA is stamped at physical route injection after source serialization. | Patent pp. 9 and 14, Appendices B and F; paper p. 4, Figure 1 | Independent per-node reproducible streams preserve source independence. Calibrated transit is added to the source-serializer completion boundary because the physical HTSIM route begins there. Mixed extents use size-normalized fixed-point event hazards so grants remain wire-byte rates; equal extents preserve the legacy sequence exactly. |
