...
public:
    ...
    uint64_t bottleneckCapacity() const { return _bottleneck_capacity_bps; }
...
private:
    uint64_t currentRateBps() const;

    uint64_t _bottleneck_capacity_bps;
...
public:
    ...
    uint64_t currentRateBps() const { return _current_rate_bps; }
    uint64_t membershipEpoch() const { return _membership_epoch; }
