...
bool sackBit(const std::array<std::uint64_t, 2>& bitmap, std::uint32_t offset) noexcept {
    return offset < kSackWindowBits &&
           (bitmap[offset / kSackWordBits] & (UINT64_C(1) << (offset % kSackWordBits))) != 0;
}

void setSackBit(std::array<std::uint64_t, 2>& bitmap, std::uint32_t offset) noexcept {
    bitmap[offset / kSackWordBits] |= UINT64_C(1) << (offset % kSackWordBits);
}

void shiftSackBitmapRightOne(std::array<std::uint64_t, 2>& bitmap) noexcept {
...
bool isReverseWireDirection(const RnicSsWirePacketMetadata& packet,
...
bool isForwardWireDirection(const RnicSsWirePacketMetadata& packet,
                            const RnicSsEndpointPair& forward_pair) {
...
bool withinHysteresis(std::uint64_t current,
                      std::uint64_t best,
...
void RnicSsPacketValidationConfig::validate() const {
    if (minimum_control_wire_bytes == 0) {
        throw std::invalid_argument("Slingshot-like control packet minimum must be nonzero");
    }
    if (maximum_wire_bytes < minimum_control_wire_bytes) {
        throw std::invalid_argument("Slingshot-like maximum wire extent is below control minimum");
    }
    if (path_count != 8) {
        throw std::invalid_argument(
    ...
    if (maximum_granted_wire_bytes_total == 0) {
        throw std::invalid_argument("Slingshot-like maximum cumulative credit must be nonzero");
    }
}

void validateRnicSsWirePacket(const RnicSsWirePacketMetadata& packet,
                              const RnicSsPacketValidationConfig& config) {
    config.validate();
    if (packet.wire_source == packet.wire_destination) {
        throw std::invalid_argument("Slingshot-like physical packet cannot loop to its source");
    }
    if (packet.wire_bytes == 0 || packet.wire_bytes > config.maximum_wire_bytes) {
        throw std::invalid_argument("Slingshot-like packet has an invalid physical wire extent");
    }

    const auto requireControlExtent = [&]() {
        if (packet.wire_bytes < config.minimum_control_wire_bytes) {
            throw std::invalid_argument("Slingshot-like control packet is below its wire minimum");
        }
    };
    ...
    switch (packet.kind) {
        case RnicSsPacketKind::DATA: {
            ...
            if (data == nullptr) {
                throw std::invalid_argument("Slingshot-like DATA kind has mismatched metadata");
            }
            ...
            if (data->payload_bytes == 0 || data->payload_bytes > packet.wire_bytes) {
                throw std::invalid_argument("Slingshot-like DATA has an invalid payload extent");
            }
        ...
        case RnicSsPacketKind::ACK_SACK: {
            ...
            if (ack == nullptr) {
                throw std::invalid_argument("Slingshot-like ACK_SACK kind has mismatched metadata");
            }
        ...
        case RnicSsPacketKind::TELEMETRY: {
            requireControlExtent();
            const auto* telemetry = std::get_if<RnicSsTelemetryMetadata>(&packet.payload);
        ...
        case RnicSsPacketKind::BP_DISABLE: {
            requireControlExtent();
            const auto* backpressure = std::get_if<RnicSsBackpressureMetadata>(&packet.payload);
            ...
            if (backpressure->control_epoch == 0) {
                throw std::invalid_argument("Slingshot-like backpressure epoch must be nonzero");
            }
            if (backpressure->granted_wire_bytes_total > config.maximum_granted_wire_bytes_total) {
                throw std::invalid_argument(
        ...
        case RnicSsPacketKind::CREDIT: {
            ...
            if (credit == nullptr) {
                throw std::invalid_argument("Slingshot-like CREDIT kind has mismatched metadata");
            }
            validatePair(credit->forward_pair);
            if (credit->congestion_domain == 0) {
                throw std::invalid_argument("Slingshot-like credit requires a physical domain");
            }
            if (!isReverseWireDirection(packet, credit->forward_pair)) {
                throw std::invalid_argument(
                    "Slingshot-like CREDIT must use the reverse physical path");
            }
            if (credit->control_epoch == 0 || credit->granted_wire_bytes_total == 0) {
                throw std::invalid_argument(
                    "Slingshot-like CREDIT requires a nonzero epoch and grant");
            }
            if (credit->granted_wire_bytes_total > config.maximum_granted_wire_bytes_total) {
                throw std::invalid_argument(
...
RnicSsSackScoreboard::RnicSsSackScoreboard(RnicSsSequence initial_next_expected)
    : _next_expected_sequence(initial_next_expected) {
    if (initial_next_expected == std::numeric_limits<RnicSsSequence>::max()) {
        throw std::invalid_argument("Slingshot-like SACK base reserves the maximum sequence value");
    }
}

RnicSsReceiveResult RnicSsSackScoreboard::observe(RnicSsSequence sequence) {
    if (sequence == std::numeric_limits<RnicSsSequence>::max()) {
        throw std::invalid_argument("Slingshot-like DATA reserves the maximum sequence value");
    }
...
RnicSsAckSackMetadata RnicSsSackScoreboard::snapshot(RnicSsEndpointPair pair) const noexcept {
...
void RnicSsSelectiveRepeatConfig::validate() const {
    if (window_packets == 0 || window_packets > kSackWindowBits) {
        throw std::invalid_argument("Slingshot-like selective-repeat window must be in [1,128]");
    }
    if (retransmission_timeout_ps == 0) {
        throw std::invalid_argument("Slingshot-like selective-repeat RTO must be nonzero");
    }
    if (maximum_retransmissions == 0) {
        throw std::invalid_argument("Slingshot-like selective-repeat requires a retry budget");
    }
}

RnicSsSelectiveRepeatLedger::RnicSsSelectiveRepeatLedger(RnicSsEndpointPair pair,
    ...
    if (initial_sequence == std::numeric_limits<RnicSsSequence>::max()) {
        throw std::invalid_argument("Slingshot-like sender reserves the maximum sequence value");
    }
}
...
RnicSsSequence RnicSsSelectiveRepeatLedger::recordNewTransmission(std::uint32_t wire_bytes,
                                                                  RnicSsTimePs now_ps) {
    if (wire_bytes == 0) {
        throw std::invalid_argument("Slingshot-like sender cannot record a zero-byte DATA packet");
    }
    if (!canSendNewPacket()) {
        throw std::logic_error("Slingshot-like selective-repeat send window is full");
    }
    const RnicSsSequence sequence = _next_sequence++;
    const bool inserted =
        _outstanding
            .emplace(sequence, RnicSsOutstandingPacket{sequence, wire_bytes, now_ps, now_ps, 1})
            .second;
    if (!inserted) {
        throw std::logic_error("Slingshot-like sender sequence already exists in ledger");
    }
    return sequence;
}

RnicSsAckResult RnicSsSelectiveRepeatLedger::applyAck(const RnicSsAckSackMetadata& ack) {
    if (ack.forward_pair != _pair) {
        throw std::invalid_argument("Slingshot-like ACK/SACK belongs to another endpoint pair");
    }
    if (sackBit(ack.sack_bitmap, 0)) {
        throw std::invalid_argument("Slingshot-like ACK/SACK bitmap is not normalized");
    }
    if (ack.next_expected_sequence < _initial_sequence) {
        throw std::invalid_argument("Slingshot-like ACK/SACK precedes this sender sequence space");
    }
    if (ack.next_expected_sequence > _next_sequence) {
        throw std::invalid_argument("Slingshot-like cumulative ACK exceeds sent sequence space");
    }
    ...
    for (std::uint32_t bit = 0; bit < kSackWindowBits; ++bit) {
        ...
        if (ack.next_expected_sequence > std::numeric_limits<RnicSsSequence>::max() - bit) {
            throw std::invalid_argument("Slingshot-like SACK sequence arithmetic overflows");
        }
        if (ack.next_expected_sequence + bit >= _next_sequence) {
            throw std::invalid_argument("Slingshot-like SACK acknowledges unsent DATA");
        }
    }
    ...
    for (auto it = _outstanding.begin(); it != _outstanding.end();) {
        ...
        if (!acknowledged && it->first >= ack.next_expected_sequence) {
            const std::uint64_t offset = it->first - ack.next_expected_sequence;
            selective_offset = offset;
            acknowledged = offset < kSackWindowBits &&
                           sackBit(ack.sack_bitmap, static_cast<std::uint32_t>(offset));
        }
        if (acknowledged) {
            result.newly_acked.push_back(it->second);
            it = _outstanding.erase(it);
        } else {
            if (highest_selective_ack.has_value() && selective_offset.has_value() &&
...
std::vector<RnicSsOutstandingPacket> RnicSsSelectiveRepeatLedger::retransmissionCandidates(
...
std::optional<RnicSsOutstandingPacket> RnicSsSelectiveRepeatLedger::retransmissionCandidate(
    ...
    if (now_ps < record.last_sent_at_ps) {
        throw std::invalid_argument("Slingshot-like retransmission time precedes DATA send");
    }
    if (now_ps - record.last_sent_at_ps < _config.retransmission_timeout_ps) {
        return std::nullopt;
    }
    ...
    if (retransmissions >= _config.maximum_retransmissions) {
        throw std::runtime_error(
            ...
            "source=" +
            std::to_string(_pair.source) + " destination=" + std::to_string(_pair.destination) +
...
void RnicSsSelectiveRepeatLedger::recordRetransmission(RnicSsSequence sequence,
                                                       RnicSsTimePs now_ps,
                                                       RnicSsRetransmissionReason reason) {
    auto it = _outstanding.find(sequence);
    if (it == _outstanding.end()) {
        throw std::out_of_range("Slingshot-like retransmission sequence is not outstanding");
    }
    RnicSsOutstandingPacket& record = it->second;
    if (now_ps < record.last_sent_at_ps) {
        throw std::logic_error("Slingshot-like retransmission precedes its previous send");
    }
    if (reason == RnicSsRetransmissionReason::RTO &&
        now_ps - record.last_sent_at_ps < _config.retransmission_timeout_ps) {
        throw std::logic_error("Slingshot-like retransmission attempted before its RTO");
    }
    if (record.transmission_count - 1 >= _config.maximum_retransmissions) {
        throw std::logic_error("Slingshot-like retransmission budget exhausted");
    }
...
void RnicSsCreditConfig::validate() const {
    if (maximum_credit_ahead_wire_bytes == 0) {
        throw std::invalid_argument("Slingshot-like credit-ahead bound must be nonzero");
    }
}

RnicSsPairCreditState::RnicSsPairCreditState(RnicSsEndpointPair pair,
                                             RnicSsCreditConfig config,
                                             RnicSsCongestionDomainId congestion_domain)
    : _pair(pair), _config(config), _congestion_domain(congestion_domain) {
    ::validatePair(pair);
    _config.validate();
    if (_congestion_domain == 0) {
        throw std::invalid_argument("Slingshot-like credit state requires a physical domain");
    }
}

void RnicSsPairCreditState::validateControl(const RnicSsEndpointPair& pair,
...
void RnicSsPairCreditState::validateCreditAhead(std::uint64_t granted_total) const {
    if (granted_total < _consumed_total ||
        granted_total - _consumed_total > _config.maximum_credit_ahead_wire_bytes) {
        throw std::invalid_argument(
...
RnicSsControlApplyResult RnicSsPairCreditState::applyEnable(
    ...
    if (enable.control_epoch == 0) {
        throw std::invalid_argument("Slingshot-like BP_ENABLE epoch must be nonzero");
    }
    if (enable.control_epoch <= _control_epoch) {
        return RnicSsControlApplyResult::DUPLICATE_OR_STALE;
    }
    if (enable.granted_wire_bytes_total > _config.maximum_credit_ahead_wire_bytes) {
        throw std::invalid_argument("Slingshot-like BP_ENABLE initial credit exceeds burst bound");
    }
    _enabled = true;
...
RnicSsControlApplyResult RnicSsPairCreditState::applyDisable(
    ...
    if (disable.control_epoch == 0) {
        throw std::invalid_argument("Slingshot-like BP_DISABLE epoch must be nonzero");
    }
    if (disable.granted_wire_bytes_total != 0) {
        throw std::invalid_argument("Slingshot-like BP_DISABLE cannot carry implicit credit");
    }
...
RnicSsControlApplyResult RnicSsPairCreditState::applyCredit(const RnicSsCreditMetadata& credit) {
    validateControl(credit.forward_pair, credit.congestion_domain);
    if (credit.control_epoch == 0 || credit.granted_wire_bytes_total == 0) {
        throw std::invalid_argument("Slingshot-like CREDIT requires a nonzero epoch and grant");
    }
...
bool RnicSsPairCreditState::canSend(std::uint32_t wire_bytes) const noexcept {
    ...
    return _granted_total >= _consumed_total && wire_bytes <= _granted_total - _consumed_total;
}

void RnicSsPairCreditState::consumeForData(std::uint32_t wire_bytes) {
    if (wire_bytes == 0) {
        throw std::invalid_argument("Slingshot-like DATA cannot consume zero wire credit");
    }
    if (!_enabled) {
        return;
    }
    if (!canSend(wire_bytes)) {
        throw std::logic_error("Slingshot-like DATA exceeds available contributor credit");
    }
...
RnicSsPairCreditTable::RnicSsPairCreditTable(RnicSsCreditConfig config) : _config(config) {
    _config.validate();
}

RnicSsPairCreditState& RnicSsPairCreditTable::stateFor(RnicSsEndpointPair pair) {
    ::validatePair(pair);
...
const RnicSsPairCreditState* RnicSsPairCreditTable::find(RnicSsEndpointPair pair) const noexcept {
...
void RnicSsPathSelectionConfig::validate() const {
    if (path_count != 8 || candidate_count != 4) {
        throw std::invalid_argument("Slingshot-like path selection requires four of eight paths");
    }
    if (maximum_sample_age_ps == 0) {
        throw std::invalid_argument("Slingshot-like load-sample lifetime must be nonzero");
    }
}

RnicSsHystereticPathSelector::RnicSsHystereticPathSelector(RnicSsPathSelectionConfig config)
    : _config(config) {
    _config.validate();
}

std::array<std::uint8_t, 4> RnicSsHystereticPathSelector::sampleFourOfEight(
    RnicSsEndpointPair pair,
    std::uint64_t selection_key) noexcept {
    std::array<std::uint8_t, 8> permutation{{0, 1, 2, 3, 4, 5, 6, 7}};
    std::uint64_t state = splitmix64((static_cast<std::uint64_t>(pair.source) << 32) |
                                     static_cast<std::uint64_t>(pair.destination));
    state = splitmix64(state ^ selection_key);
    for (std::size_t i = 0; i < 4; ++i) {
        state = splitmix64(state + i);
        const std::size_t selected = i + static_cast<std::size_t>(state % (8 - i));
        std::swap(permutation[i], permutation[selected]);
    }
    return {{permutation[0], permutation[1], permutation[2], permutation[3]}};
}

bool RnicSsHystereticPathSelector::ingestLoadSample(const RnicSsDelayedLoadSample& sample,
                                                    RnicSsTimePs received_at_ps) {
    if (sample.path_id >= _config.path_count) {
        throw std::invalid_argument("Slingshot-like telemetry names an unavailable path");
    }
    if (received_at_ps < sample.observed_at_ps) {
        throw std::invalid_argument("Slingshot-like telemetry arrived before it was observed");
    }
...
std::pair<std::uint64_t, bool> RnicSsHystereticPathSelector::loadFor(std::uint8_t path_id,
                                                                     RnicSsTimePs now_ps) const {
...
RnicSsPathDecision RnicSsHystereticPathSelector::select(
    ...
    if (current_path.has_value() && *current_path >= _config.path_count) {
        throw std::invalid_argument("Slingshot-like current path is outside the physical path set");
    }
    ...
    if (current_path.has_value()) {
        auto current =
            std::find(decision.candidates.begin(), decision.candidates.end(), *current_path);
        if (current != decision.candidates.end()) {
            const std::size_t current_index =
                static_cast<std::size_t>(std::distance(decision.candidates.begin(), current));
            if (withinHysteresis(decision.candidate_queue_delay_ps[current_index],
                                 decision.candidate_queue_delay_ps[best_index],
                                 _config.hysteresis_queue_delay_ps)) {
                selected_index = current_index;
                decision.retained_by_hysteresis = current_index != best_index;
            }
        }
    }
