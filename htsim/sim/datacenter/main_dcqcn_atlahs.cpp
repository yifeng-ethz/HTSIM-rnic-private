...
std::optional<AtlahsHtsimApi::GoalRankMapping> rankMappingOverride(
        DcqcnGoalRankMapping mapping) {
    switch (mapping) {
...
const char* resolvedRankMappingName(
...
void writeCompletionCsv(
    ...
    for (const AtlahsCompletedFlowRecord& flow : ordered) {
        output
            ...
            << flow.tag << ',' << flow.payload_bytes << ','
            << flow.start_time_ps << ',' << flow.completion_time_ps << ','
            << flow.fct_ps() << '\n';
    }
    output.flush();
    if (!output) {
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        const DcqcnAtlahsCliOptions options =
            parseDcqcnAtlahsCli(argc, arguments.data());
        ...
        api.setGoalRankMappingOverride(
            rankMappingOverride(options.goal_rank_mapping));
        ...
        const int result = start_lgs(
            ...
            [&](const AtlahsHtsimApi::GoalLayout& layout) {
                auto runtime = std::make_unique<DcqcnAtlahsRuntime>(
                    event_list,
                    options.runtime,
        ...
        if (api.runtimeHasPendingPhysicalWork()) {
            throw std::logic_error(
                "ATLAHS returned before DCQCN physical quiescence");
        }
        ...
        std::cout
            << "[DCQCN manifest] completed_flows="
            << installed_runtime->completed_flow_count()
            << " silent_rtos=" << installed_runtime->silent_rto_count()
            << " ecn_marked_packets="
            << installed_runtime->ecn_marked_packet_count()
            << " pfc_pause_frames="
            << installed_runtime->pfc_pause_count()
            << " pfc_resume_frames="
            << installed_runtime->pfc_resume_count()
            << " ns_tm3_dropped_packets="
            << installed_runtime->dropped_packet_count()
