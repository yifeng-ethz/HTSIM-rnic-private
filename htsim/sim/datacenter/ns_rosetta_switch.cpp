...
NsRosettaIngressPort::NsRosettaIngressPort(NsRosetta& owner,
                                           uint32_t ingress_id,
                                           std::string name)
    : _owner(owner),
...
NsRosettaEgressSerializer::NsRosettaEgressSerializer(
    linkspeed_bps bitrate, EventList& eventlist, QueueLogger* logger)
    : BaseQueue(bitrate, eventlist, logger) {
    _nodename = "ns-rosetta-egress-serializer";
}

void NsRosettaEgressSerializer::bind(NsRosetta& owner,
                                     uint32_t egress_id) {
    if (_owner != nullptr) {
        throw std::logic_error(
            "ns-rosetta egress serializer already bound");
    }
    _owner = &owner;
...
void NsRosettaEgressSerializer::receivePacket(Packet& pkt) {
    ...
    if (_authorized_dispatch != &pkt) {
        throw std::logic_error(
            "ns-rosetta egress serializer cannot bypass request/grant");
    }
    if (_packet_in_service != nullptr) {
        throw std::logic_error(
            "ns-rosetta request/grant dispatched onto a busy serializer");
    }
    ...
    if (EventList::now() >
        std::numeric_limits<simtime_picosec>::max() - serialization_time) {
        _packet_in_service = nullptr;
        throw std::overflow_error(
            "ns-rosetta serialization completion time overflow");
    }
...
void NsRosettaEgressSerializer::dispatch(Packet& pkt) {
    if (_authorized_dispatch != nullptr) {
        throw std::logic_error(
            "ns-rosetta egress serializer has a pending dispatch");
    }
...
void NsRosettaEgressSerializer::doNextEvent() {
    if (_packet_in_service == nullptr) {
        throw std::logic_error(
            "idle ns-rosetta serializer received an event");
    }
...
simtime_picosec NsRosettaEgressSerializer::remaining_service_time_ps() const {
    if (_packet_in_service == nullptr ||
        _service_completion_time <= EventList::now()) {
        return 0;
    }
    return _service_completion_time - EventList::now();
}

simtime_picosec NsRosettaEgressSerializer::serialization_delay_for_bytes(
    mem_b bytes) const {
    if (bytes < 0) {
        throw std::invalid_argument(
            "ns-rosetta cannot serialize a negative byte count");
    }
    const auto unsigned_bytes = static_cast<uint64_t>(bytes);
    if (_ps_per_byte != 0 &&
        unsigned_bytes >
            std::numeric_limits<simtime_picosec>::max() / _ps_per_byte) {
        throw std::overflow_error(
            "ns-rosetta backlog serialization delay overflow");
    }
...
NsRosetta::NsRosetta(EventList& eventlist, const string& name,
...
int NsRosetta::addPort(BaseQueue* queue) {
    ...
    if (serializer == nullptr) {
        throw std::invalid_argument(
            "ns-rosetta switch ports must be physical egress serializers");
    }
...
PacketSink* NsRosetta::create_physical_ingress(const string& name) {
    ...
    IngressState state;
    state.port =
        std::make_unique<NsRosettaIngressPort>(*this, ingress_id, name);
    PacketSink* result = state.port.get();
    _ingresses.push_back(std::move(state));
    return result;
}

void NsRosetta::receive_from_physical_ingress(Packet& pkt,
                                              uint32_t ingress_id) {
    if (pkt.type() == ETH_PAUSE) {
        throw std::invalid_argument(
            "ns-rosetta does not support PAUSE/PFC packets");
    }
    validate_ingress(ingress_id);
    if (!_pipeline_ingress.emplace(&pkt, ingress_id).second) {
        throw std::logic_error(
            "packet entered the ns-rosetta switch pipeline twice");
    }
...
void NsRosetta::receivePacket(Packet& pkt) {
    ...
    if (pending == _pipeline_ingress.end()) {
        throw std::logic_error(
            "ns-rosetta packets must enter through a physical ingress");
    }
...
NsRosetta::PacketSummary NsRosetta::summarize(
    Packet& pkt, uint32_t ingress_id, uint32_t egress_id,
    size_t selected_traffic_class) {
    return PacketSummary{ingress_id,
                         egress_id,
...
NsRosettaEgressSerializer& NsRosetta::resolve_selected_egress(Packet& pkt) {
    if (pkt.route() != nullptr && pkt.nexthop() < pkt.route()->size()) {
        auto* egress = dynamic_cast<NsRosettaEgressSerializer*>(
            pkt.route()->at(pkt.nexthop()));
    ...
    auto* egress =
        dynamic_cast<NsRosettaEgressSerializer*>(selected_route->at(0));
    if (egress == nullptr || egress->owner() != this) {
        throw std::logic_error(
            "ns-rosetta FIB did not select a local physical egress");
    }
    return *egress;
}

void NsRosetta::enqueue(Packet& pkt, uint32_t ingress_id,
                        NsRosettaEgressSerializer& egress) {
    const size_t selected_traffic_class = traffic_class(pkt.priority());
    const PacketSummary packet = summarize(
        pkt, ingress_id, egress.egress_id(), selected_traffic_class);

    if (packet.packet_bytes > _shared_buffer_capacity ||
        _shared_buffer_occupancy >
            _shared_buffer_capacity - packet.packet_bytes) {
    ...
    Voq& voq = output.traffic_classes[selected_traffic_class]
                    .by_ingress[ingress_id];
...
void NsRosetta::run_arbitration() {
    ...
    while (true) {
        ...
        for (uint32_t egress_id = 0; egress_id < _egresses.size();
             ++egress_id) {
            ...
            const std::optional<Grant> grant =
                select_egress_grant(egress_id, ingress_matched);
            if (grant.has_value()) {
        ...
        for (uint32_t ingress_id = 0; ingress_id < _ingresses.size();
             ++ingress_id) {
            if (ingress_matched[ingress_id] ||
                ingress_state(ingress_id).grant_busy ||
                grants_by_ingress[ingress_id].empty()) {
                continue;
            }
            const Grant accepted =
                accept_input_grant(ingress_id,
                                   grants_by_ingress[ingress_id]);
...
std::optional<NsRosetta::Grant> NsRosetta::select_egress_grant(
    ...
    for (size_t selected_class = 0;
         selected_class < egress.traffic_classes.size(); ++selected_class) {
        const TrafficClassVoqs& traffic_class_state =
            egress.traffic_classes[selected_class];
        ...
        auto first = traffic_class_state.has_last_granted_ingress
                         ? requests.upper_bound(
                               traffic_class_state.last_granted_ingress)
                         : requests.begin();
        ...
        do {
            ...
            if (!candidate->second.packets.empty() &&
                !ingress_matched[ingress_id] &&
                !ingress_state(ingress_id).grant_busy) {
...
NsRosetta::Grant NsRosetta::accept_input_grant(
    uint32_t ingress_id, const std::vector<Grant>& grants) const {
    if (grants.empty()) {
        throw std::invalid_argument(
            "ns-rosetta input cannot accept an empty grant set");
    }
    const IngressState& ingress = ingress_state(ingress_id);
    const size_t highest_class =
        std::min_element(
            grants.begin(), grants.end(),
            [](const Grant& left, const Grant& right) {
    ...
    for (const Grant& grant : grants) {
        ...
        if (ingress.has_last_accepted_egress &&
            grant.egress_id > ingress.last_accepted_egress &&
            (after_cursor == nullptr ||
             grant.egress_id < after_cursor->egress_id)) {
            after_cursor = &grant;
        }
    }
...
void NsRosetta::dispatch_grant(const Grant& grant) {
    ...
    TrafficClassVoqs& traffic_class_state =
        egress.traffic_classes[grant.traffic_class];
    auto request = traffic_class_state.by_ingress.find(grant.ingress_id);
    if (request == traffic_class_state.by_ingress.end() ||
        request->second.packets.empty()) {
        throw std::logic_error("ns-rosetta granted an empty VoQ");
    }
    if (ingress.grant_busy || egress.serializer->is_busy() ||
        egress.active_grant.has_value()) {
        throw std::logic_error("ns-rosetta request/grant match is not free");
    }

    Packet* packet = request->second.packets.front();
    const PacketSummary summary = summarize(
        *packet, grant.ingress_id, grant.egress_id, grant.traffic_class);
    if (packet->route() == nullptr ||
        packet->nexthop() >= packet->route()->size() ||
        packet->route()->at(packet->nexthop()) != egress.serializer) {
        throw std::logic_error(
            "ns-rosetta granted packet does not target its serializer");
    }
    if (request->second.buffered_bytes < summary.packet_bytes ||
        ...
        throw std::logic_error(
            "ns-rosetta shared-buffer accounting underflow");
    }
    ...
    if (egress_pair == egress.pairs.end() ||
        ...
        throw std::logic_error(
            "ns-rosetta egress-pair accounting underflow");
    }
...
void NsRosetta::egress_serialization_complete(uint32_t egress_id) {
    ...
    if (!egress.active_grant.has_value()) {
        throw std::logic_error(
            "ns-rosetta serializer completed without an active grant");
    }
    const PacketSummary summary = *egress.active_grant;
    IngressState& ingress = ingress_state(summary.ingress_id);
    if (!ingress.grant_busy ||
        ingress.in_service_bytes < summary.packet_bytes) {
        throw std::logic_error(
            "ns-rosetta ingress grant accounting underflow");
    }
    auto pair = _pairs.find(summary.pair);
    if (pair == _pairs.end() ||
        pair->second.in_service_bytes < summary.packet_bytes) {
        throw std::logic_error(
            "ns-rosetta pair service accounting underflow");
    }
    auto egress_pair = egress.pairs.find(summary.pair);
    if (egress_pair == egress.pairs.end() ||
        egress_pair->second.in_service_bytes < summary.packet_bytes) {
        throw std::logic_error(
            "ns-rosetta egress-pair service accounting underflow");
    }
    ...
    emit_transition(NsRosettaQueueTransition::SerializationCompleted,
                    summary);
    if (pair->second.buffered_bytes == 0 &&
        pair->second.in_service_bytes == 0) {
        _pairs.erase(pair);
    }
    if (egress_pair->second.buffered_bytes == 0 &&
        egress_pair->second.in_service_bytes == 0) {
        egress.pairs.erase(egress_pair);
    }
    run_arbitration();
}

void NsRosetta::emit_transition(NsRosettaQueueTransition transition,
    ...
    observer->observe(NsRosettaBacklogObservation{
        ...
        voq_buffered_bytes(packet.ingress_id, packet.egress_id,
                           packet.traffic_class == 0
                               ? Packet::PRIO_HI
                               : packet.traffic_class == 1
                                     ? Packet::PRIO_MID
                                     : Packet::PRIO_LO),
        ingress_buffered_bytes(packet.ingress_id),
        ingress_backlog_bytes(packet.ingress_id),
        egress_buffered_bytes(packet.egress_id),
        egress_backlog_bytes(packet.egress_id),
        egress_pair_buffered_bytes(packet.egress_id, packet.pair),
        egress_pair_backlog_bytes(packet.egress_id, packet.pair),
        shared_buffer_occupancy()});
}

mem_b NsRosetta::voq_buffered_bytes(
    uint32_t ingress_id, uint32_t egress_id,
    Packet::PktPriority priority) const {
    validate_ingress(ingress_id);
    const EgressState& egress = egress_state(egress_id);
    const auto& requests = egress.traffic_classes[traffic_class(priority)]
                               .by_ingress;
...
mem_b NsRosetta::egress_pair_buffered_bytes(
...
mem_b NsRosetta::egress_pair_backlog_bytes(
    uint32_t egress_id, const NsRosettaEndpointPair& pair) const {
    const EgressState& egress = egress_state(egress_id);
    auto state = egress.pairs.find(pair);
    return state == egress.pairs.end()
               ? 0
               : state->second.buffered_bytes +
                     state->second.in_service_bytes;
}

mem_b NsRosetta::pair_buffered_bytes(
    const NsRosettaEndpointPair& pair) const {
    auto state = _pairs.find(pair);
    return state == _pairs.end() ? 0 : state->second.buffered_bytes;
}

mem_b NsRosetta::pair_backlog_bytes(
    const NsRosettaEndpointPair& pair) const {
    auto state = _pairs.find(pair);
    return state == _pairs.end()
               ? 0
               : state->second.buffered_bytes +
                     state->second.in_service_bytes;
}

NsRosettaRequestDepth NsRosetta::request_depth(
...
NsRosettaPathLoad NsRosetta::sample_path_load(uint32_t egress_id) const {
    ...
    for (const TrafficClassVoqs& traffic_class_state :
         egress.traffic_classes) {
    ...
    const simtime_picosec buffered_delay =
        egress.serializer->serialization_delay_for_bytes(
            egress.buffered_bytes);
    const simtime_picosec remaining_service =
        egress.serializer->remaining_service_time_ps();
    if (remaining_service >
        std::numeric_limits<simtime_picosec>::max() - buffered_delay) {
        throw std::overflow_error(
            "ns-rosetta sampled path-load delay overflow");
    }
    return NsRosettaPathLoad{
        EventList::now(),
        egress_id,
...
const NsRosetta::IngressState& NsRosetta::ingress_state(
    uint32_t ingress_id) const {
    validate_ingress(ingress_id);
    return _ingresses[ingress_id];
}
...
const NsRosetta::EgressState& NsRosetta::egress_state(
