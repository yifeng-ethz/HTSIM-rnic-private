...
std::uint16_t RnicCollectivePacket::checkedWireBytes(
        std::uint64_t wire_bytes) {
    if (wire_bytes == 0
        || wire_bytes > std::numeric_limits<std::uint16_t>::max()) {
        throw std::invalid_argument(
            "rnic-cn packet wire extent must fit exactly in uint16_t");
    }
...
void RnicCollectivePacket::validateRoute(const Route& route) {
    if (route.size() == 0) {
        throw std::invalid_argument(
            "rnic-cn packet requires a nonempty explicit route");
    }
    if (route.size() > std::numeric_limits<std::uint32_t>::max()) {
        throw std::overflow_error(
            "rnic-cn packet route length exceeds the HTSIM hop ledger");
    }
}

void RnicCollectivePacket::validateFinalLedger(
        const RnicCollectiveFinalLedger& final_ledger) {
    if (final_ledger.total_data_packets == 0) {
        if (final_ledger.total_payload_bytes != 0
            || final_ledger.total_wire_bytes != 0) {
            throw std::invalid_argument(
                "rnic-cn empty DATA ledger must have zero byte totals");
        }
        return;
    }
    ...
    if (final_ledger.total_payload_bytes == 0) {
        throw std::invalid_argument(
            "rnic-cn nonempty DATA ledger requires payload bytes");
    }
    if (final_ledger.total_wire_bytes
        < final_ledger.total_payload_bytes) {
        throw std::invalid_argument(
            "rnic-cn final wire ledger cannot be smaller than payload");
    }
    if (final_ledger.total_data_packets
        > final_ledger.total_payload_bytes) {
        throw std::invalid_argument(
            "rnic-cn DATA ledger requires positive payload in every packet");
    }
    if (final_ledger.total_data_packets
        > final_ledger.total_wire_bytes) {
        throw std::invalid_argument(
            "rnic-cn DATA ledger requires positive wire extent per packet");
    }
}

void RnicCollectivePacket::validateData(
    ...
    if (ledger.total_data_packets == 0) {
        throw std::invalid_argument(
            "rnic-cn DATA packet cannot use an empty final ledger");
    }
    if (metadata.extent.payloadBytes() == 0) {
        throw std::invalid_argument(
            "rnic-cn DATA packet must advance the payload ledger");
    }
    if ((metadata.transmission_attempt == 0)
        != !metadata.repairs_lifecycle_id.has_value()) {
        throw std::invalid_argument(
            "rnic-cn DATA repair attempt and replaced lifecycle must agree");
    }
    if (metadata.repairs_lifecycle_id.has_value()
        && *metadata.repairs_lifecycle_id == 0) {
        throw std::invalid_argument(
            "rnic-cn DATA cannot repair lifecycle zero");
    }
    checkedWireBytes(metadata.extent.wireBytes());
    if (metadata.packet_index >= ledger.total_data_packets) {
        throw std::out_of_range(
            "rnic-cn DATA packet index exceeds its final ledger");
    }
    if ((metadata.packet_index == 0)
        != (metadata.payload_byte_offset == 0)) {
        throw std::invalid_argument(
            "rnic-cn DATA packet zero index and zero offset must agree");
    }
    if (metadata.payload_byte_offset < metadata.packet_index) {
        throw std::invalid_argument(
            "rnic-cn DATA offset cannot fit preceding positive payloads");
    }
    if (metadata.payload_byte_offset > ledger.total_payload_bytes
        ...
        throw std::invalid_argument(
            "rnic-cn DATA payload extent exceeds its final ledger");
    }

    const std::uint64_t payload_end =
        metadata.payload_byte_offset + metadata.extent.payloadBytes();
    const std::uint64_t remaining_payload =
        ledger.total_payload_bytes - payload_end;
    const std::uint64_t remaining_packets =
        ledger.total_data_packets - metadata.packet_index - 1;
    if (remaining_payload < remaining_packets) {
        throw std::invalid_argument(
            "rnic-cn DATA final ledger cannot fit remaining packets");
    }
    if ((remaining_packets == 0) != (remaining_payload == 0)) {
        throw std::invalid_argument(
            "rnic-cn final DATA index and payload end must agree");
    }

    const std::uint64_t other_packets = ledger.total_data_packets - 1;
    if (metadata.extent.wireBytes() > ledger.total_wire_bytes
        || other_packets
               > ledger.total_wire_bytes - metadata.extent.wireBytes()) {
        throw std::invalid_argument(
            "rnic-cn DATA wire extent cannot fit its final ledger");
    }
}
...
void RnicCollectivePacket::validateGrant(
    ...
    if (packet_kind == RnicCollectivePacketKind::ACCEPT) {
        ...
        throw std::logic_error(
            "rnic-cn grant validation requires a grant packet kind");
    }

    if (grant.kind != expected_kind) {
        throw std::invalid_argument(
            "rnic-cn grant kind does not match its wire packet kind");
    }
    if (grant.membership_epoch == 0) {
        throw std::invalid_argument(
            "rnic-cn in-band grant requires a nonzero membership epoch");
    }
    if (grant.n_hat == 0 || grant.wire_rate_bps == 0) {
        throw std::invalid_argument(
            "rnic-cn in-band grant requires positive N_hat and wire rate");
    }
}

void RnicCollectivePacket::validateGapNack(
        const RnicCollectiveGapNackMetadata& gap_nack) {
    if (gap_nack.extent.payloadBytes() == 0
        || gap_nack.extent.wireBytes() == 0) {
        throw std::invalid_argument(
            "rnic-cn GAP_NACK requires a nonempty logical packet");
    }
    checkedWireBytes(gap_nack.extent.wireBytes());
    if (gap_nack.rejected_lifecycle_id == 0
        || gap_nack.requested_transmission_attempt == 0) {
        throw std::invalid_argument(
            "rnic-cn GAP_NACK requires a rejected lifecycle and retry");
    }
}
...
RnicCollectivePacket* RnicCollectivePacket::newAccept(
    ...
    return newPacket(
        ...
        route,
        htsim_packet_id,
        RnicCollectivePacketKind::ACCEPT,
        grant.flow_id,
        source,
        destination,
        checkedWireBytes(wire_bytes),
...
RnicCollectivePacket* RnicCollectivePacket::newPacket(
    ...
    if (!observer) {
        throw std::invalid_argument(
            "rnic-cn packet requires a lifecycle observer");
    }
    ...
    case RnicCollectivePacketKind::DATA:
        if (!has_data || has_declaration || has_grant || has_gap_nack
            || has_retire_ledger) {
    ...
    case RnicCollectivePacketKind::GAP_NACK:
        if (has_data || has_declaration || has_grant || !has_gap_nack
            || has_retire_ledger) {
            throw std::logic_error(
                "rnic-cn GAP_NACK metadata shape is invalid");
        }
        break;
...
void RnicCollectivePacket::initialize(
    ...
    if (_pool_state != PoolState::RECYCLED || ref_count() != 1) {
        throw std::logic_error(
            "rnic-cn packet pool returned a live or shared packet");
    }
...
const RnicCollectiveGapNackMetadata&
RnicCollectivePacket::gapNack() const {
    if (!_gap_nack.has_value()) {
        throw std::logic_error(
            "rnic-cn packet does not carry GAP_NACK metadata");
    }
    return *_gap_nack;
}

const RnicCollectiveFinalLedger&
RnicCollectivePacket::finalLedger() const {
    if (_data.has_value()) {
    ...
    throw std::logic_error(
        "rnic-cn packet does not carry a final DATA ledger");
}
...
Packet::PktPriority RnicCollectivePacket::priority() const {
    return _kind == RnicCollectivePacketKind::DATA
        ? Packet::PRIO_LO
        : Packet::PRIO_HI;
}
...
RnicCollectivePacketObservation RnicCollectivePacket::observation(
    ...
    return {
        ...
        nexthop(),
        _data.has_value()
            ? std::optional<std::uint64_t>(_data->packet_index)
            : std::nullopt,
    };
}
...
void RnicCollectivePacket::consumeAtEndpoint() {
    if (_pool_state != PoolState::IN_FLIGHT) {
        throw std::logic_error(
            "rnic-cn packet endpoint consumption must occur exactly once");
    }
    if (route() == nullptr || nexthop() != route()->size()) {
        throw std::logic_error(
            "rnic-cn packet cannot be consumed before its explicit route ends");
    }
...
void RnicCollectivePacket::terminate(
        RnicCollectivePacketLifecycle lifecycle) {
    if (lifecycle == RnicCollectivePacketLifecycle::CREATED) {
        throw std::logic_error(
            "rnic-cn CREATED is not a terminal packet lifecycle");
    }
    if (_pool_state != PoolState::IN_FLIGHT) {
        throw std::logic_error(
            "rnic-cn packet terminal operation must occur exactly once");
    }
    if (ref_count() != 1) {
        throw std::logic_error(
            "rnic-cn packet primitive does not support shared references");
    }
    if (!_observer) {
        throw std::logic_error(
            "rnic-cn packet cannot terminate without its observer");
    }
    if (size() != _wire_bytes) {
        throw std::logic_error(
            "rnic-cn packet wire size changed while it was in flight");
    }
...
void RnicCollectivePacket::bounce() {
    throw std::logic_error(
        "rnic-cn packets have no bounce or retransmission path");
}
...
void RnicCollectivePacket::set_route(const Route&) {
    throw std::logic_error(
        "rnic-cn packet explicit route is immutable in flight");
}
...
void RnicCollectivePacket::set_route(
        PacketFlow&, const Route&, int, packetid_t) {
    throw std::logic_error(
        "rnic-cn packet explicit route is immutable in flight");
}
